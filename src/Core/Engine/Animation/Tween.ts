...
class Tween {
  ...
  private _name: string; _symbol: Symbol; _easing: string; _object: any; _data: TWEEN.Tween<any> | null;
  ...
  public init(name: string, easing: string, object: any, repeat: number = 0, delay: number = 0) {
    this._name = name;
   ...
   // if (delay !== 0) this._data.delay(delay);
    ...
    this._loop.addFunction(this._symbol, this.update, this);
  }
  ...
  public destroy() {
    ...
    this._loop.removeFunction(this._symbol);
  }

  public to(toObject: any, time: number, updateFunction: Function = () => { }, completeFunction = () => {}): Tween {
    this.freeze();
  ...
  public createNew(): Tween {
    return new Tween(this._loop);
  }
